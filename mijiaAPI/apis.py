...
class mijiaAPI():
    ...
    # ================= 新增：Web 友好型登录方法 =================
    def start_web_login(self) -> str:
        """
        启动 Web 登录流程（非阻塞）
        返回: 二维码图片链接 (URL)
        如果 Token 已有效，返回 None
        """
        # 如果已登录，直接返回
        if self.available:
            return None

        # Step 1: 获取 location 参数
        location_data = self._get_location()
        ...
        # 保存登录会话状态，供后续轮询使用
        self._web_login_session = requests.Session()
        self._web_login_lp_url = login_data["lp"]
        self._web_login_headers = headers
        self._web_login_start_time = time.time()
        self._web_login_active = True

        return login_data["qr"]

    def check_web_login_status(self) -> dict:
        """
        检查 Web 登录状态（非阻塞，适合前端轮询）
        返回: {"status": "waiting"|"success"|"timeout"|"error", "message": str}
        """
        if not getattr(self, '_web_login_active', False):
            return {"status": "error", "message": "未调用 start_web_login()"}

        # 检查是否超时（115秒，留5秒缓冲）
        if time.time() - self._web_login_start_time > 115:
            self._web_login_active = False
            return {"status": "timeout", "message": "二维码已过期，请重新获取"}

        try:
            # 使用短超时进行轮询（长轮询服务器会在扫码后立刻返回，未扫码会阻塞）
            lp_ret = self._web_login_session.get(
                self._web_login_lp_url, 
                headers=self._web_login_headers, 
                timeout=2
            )
            lp_data = self._handle_ret(lp_ret)

            # 扫码成功，执行 Step 4 保存 Token
            auth_keys = ["psecurity", "nonce", "ssecurity", "passToken", "userId", "cUserId"]
            for key in auth_keys:
                self.auth_data[key] = lp_data[key]
            
            callback_url = lp_data["location"]
            self._web_login_session.get(callback_url, headers=self._web_login_headers)
            cookies = self._web_login_session.cookies.get_dict()
            self.auth_data.update(cookies)
            self.auth_data.update({
                "expireTime": int((datetime.now() + timedelta(days=30)).timestamp() * 1000),
            })
            ...
            self._web_login_active = False
            return {"status": "error", "message": f"登录异常: {str(e)}"}
